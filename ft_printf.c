...
/*   Updated: 2022/04/19 18:48:53 by stales           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_va(char *fmt, va_list *a)
{
	...
	if (*fmt == 's')
		return (ft_putstr_fd(va_arg(*a, char *), 1));
	if (*fmt == 'd' || *fmt == 'i')
	...
	if (*fmt == 'p')
	{
		...
		if (addr)
			return (ft_putstr_fd("0x", 1) + ft_puthex_fd(addr, 1, *fmt));
...
int	ft_printf(const char *format, ...)
{
	...
	while (*format)
	{
		...
		save = (char *)format;
		format += ft_putstr_fd_end((char *)format, 1, '%');
		i += format - save;
	}
	va_end(args);
	return (i);
